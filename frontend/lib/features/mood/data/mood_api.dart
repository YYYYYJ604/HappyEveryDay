...
class MoodApi {
  ...
  /// POST /moods — 记录心情
  Future<ApiResponse<Map<String, dynamic>>> create(Map<String, dynamic> body) async {
    final resp = await _dio.post(ApiEndpoints.moods, data: body);
    return ApiResponse.fromJson(resp.data, null);
  }

  /// GET /moods — 查询心情历史
  Future<ApiResponse<List<dynamic>>> getHistory(Map<String, dynamic> query) async {
  ...
  Future<ApiResponse<Map<String, dynamic>>> getMonthlyStats(int year, int month) async {
    final resp = await _dio.get('', queryParameters: {'year': year, 'month': month});
    return ApiResponse.fromJson(resp.data, null);
  }
}
